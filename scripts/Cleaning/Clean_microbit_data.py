...
def MicrobitDataCleaner(raw_df):
    ...
    df_raw.to_csv("data/MicrobitDataClean.csv", index=False)
    ...
    df_h.to_csv("data/MicrobitDataHourlyClean.csv", index=False)
    ...
    df_daily.to_csv("data/MicrobitDataDailyClean.csv", index=False)
    print(f"Daily export complete, {len(df_daily)} rows at data/MicrobitDataDailyClean.csv")
...
if __name__ == "__main__":
    while True:
        ...
        if filepath == "":
            filepath = "data\\MicrobitDataUncleaned.csv"
        try:
